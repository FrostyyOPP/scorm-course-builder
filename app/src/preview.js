...
(async () => {
  ...
  cf.on('error', (e) => {
    if (e.code === 'ENOENT') console.log('\n⚠️  cloudflared not found — public link unavailable. Install: ' +
      (process.platform === 'win32' ? 'winget install Cloudflare.cloudflared  (or: scoop install cloudflared)' : 'brew install cloudflared') +
